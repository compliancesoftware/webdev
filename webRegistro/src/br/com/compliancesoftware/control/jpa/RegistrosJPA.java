...
public class RegistrosJPA implements RegistrosDao
{
	...
	public List<Registro> lista(FiltroRegistro filtro) 
	{
		...
		Calendar iIni = filtro.getDtInclusaoInicio();
		Calendar iFim = filtro.getDtInclusaoFim();
		if((iIni != null) && (iFim != null))
		{
			q += " and r.incluido >= :paramIncIni and r.incluido <= :paramIncFim";
			
			param = new HashMap<String,Object>();
			param.put("tipo", "Calendar");
			param.put("nome", "paramincIni");
			param.put("valor", iIni);
			params.add(param);
			
			param = new HashMap<String,Object>();
			param.put("tipo", "Calendar");
			param.put("nome", "paramIncFim");
			param.put("valor", iFim);
			params.add(param);
		}
		else if((iIni == null) && (iFim != null))
		{
			q += " and r.incluido <= :paramIncFim";
		...
		String quem_incluiu = filtro.getQuemIncluiu();
		if(quem_incluiu != null && !quem_incluiu.equals(""))
		{
			quem_incluiu = "%"+quem_incluiu+"%";
			q += " and r.quem_incluiu.nome like :paramQIncluiu";
			
			param = new HashMap<String,Object>();
			param.put("tipo", "String");
			param.put("nome", "paramQIncluiu");
			param.put("valor", quem_incluiu);
			params.add(param);
		}
		
		int vencIni = filtro.getVencimentoInicio();
		int vencFim = filtro.getVencimentoFim();
		
		if(vencIni > 0 && vencFim <= 30)
		{
			q += " and r.vencimento >= :paramVencIni and r.vencimento <= :paramVencFim";
			
			param = new HashMap<String,Object>();
			param.put("tipo", "int");
			param.put("nome", "paramVencIni");
			param.put("valor", vencIni);
			params.add(param);
			
			param = new HashMap<String,Object>();
			param.put("tipo", "int");
			param.put("nome", "paramVencFim");
			param.put("valor", vencFim);
			params.add(param);
		}
